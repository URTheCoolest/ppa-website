...
export default function AdminUploadPage() {
  ...
  const validateFiles = (): string | null => {
    ...
    for (const file of files) {
      ...
      if (!file.description || file.description.trim().length < 15) return 'Please add a description (minimum 15 characters) for all files'
    }
    return null
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingMedia && existingMedia.length > 0) {
        const lastId = existingMedia[0].media_id
        const match = lastId.match(/PPA-MEDIA-(\d+)/)
        if (match) {
